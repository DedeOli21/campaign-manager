...
export function validateCampaignDates(startDate: Date, endDate: Date, status?: CampaignStatus): CampaignStatus {
    const now = new Date();
    ...
    return status && Object.values(CampaignStatus).includes(status) ? status : CampaignStatus.ACTIVE;
  }